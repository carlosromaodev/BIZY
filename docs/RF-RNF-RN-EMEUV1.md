...
| RF267 | [~] Webhooks, importações, campanhas e eventos públicos devem usar chaves de idempotência para evitar duplicidade. | Alta | Parcial - eventos operacionais, jobs de importação/exportação de clientes/produtos, eventos públicos de tracking e webhook Evolution suportam idempotência por chave técnica; faltam conectores sociais externos, campanhas públicas e persistência cross-process para todos os webhooks |
...
| RNF69 | [~] O sistema deve manter idempotência em importações, campanhas e webhooks para evitar duplicação de clientes, pedidos ou mensagens. | Alta | Parcial - importações/exportações por job, eventos operacionais, tracking público e webhook Evolution têm idempotência; faltam campanhas externas e persistência distribuída para todos os webhooks |
...
| RNF77 | [~] Webhooks de redes sociais e WhatsApp devem ser idempotentes para evitar duplicar clientes, comentários, mensagens, pedidos ou comissões. | Alta | Parcial - Evolution deduplica callbacks repetidos por provider/instância/direção/id/status dentro do processo, comentários usam ids quando disponíveis e eventos operacionais têm idempotencyKey; faltam conectores sociais oficiais e ledger persistente para retries entre réplicas |
