import { ArrowLeft, Heart, Loader2, MapPin, Package, Star, Store } from "lucide-react";
import { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import { obterCatalogoPublico, normalizarProdutoLojaPublica, ROTAS_LOJAS } from "../lojas";
import type { RespostaCatalogoPublico } from "../lojas";
...
function obterResumoComercial() {
  return { rating: "4.8", avaliacoes: 0, vendidos: 0 };
}
...
function CartaoProdutoCatalogo({ produto }: { produto: ProdutoMarketNormalizado }) {
  const resumo = obterResumoComercial();
  return (
    <article className="market-product-card">
      ...
      <div className="market-product-body">
        ...
        <div className="market-product-rating" aria-label="Avaliação">
          <Star size={13} />
          <strong>{resumo.rating}</strong>
          <span>({resumo.avaliacoes})</span>
        </div>
