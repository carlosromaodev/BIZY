...
function ConteudoCreator({ rota, dados, atualizar }: { rota: string; dados: CreatorPortalDados; atualizar: () => Promise<void> }) {
  ...
  if (rota === "/creator/carrinhos") return <CarrinhosCreator dados={dados} />;
...
function TabelaComissoes({ dados }: { dados: CreatorPortalDados }) { const movimentos = dados.ledger?.movimentos ?? []; return <section className="creator-panel"><h2>Movimentos de comissão</h2><div className="creator-table-wrap"><table><thead><tr><th>Movimento</th><th>Saldo</th><th>Motivo</th><th>Valor</th></tr></thead><tbody>{movimentos.map((m) => <tr key={m.id}><td>{m.tipo.replace(/_/g, " ")}</td><td>{m.saldoOrigem ? `${m.saldoOrigem} → ` : ""}{m.saldoDestino}</td><td>{m.motivo}</td><td>{formatarKwanza(m.valorEmKwanza)}</td></tr>)}</tbody></table></div>{!movimentos.length && <p className="creator-empty">Nenhum movimento registado.</p>}</section>; }
...
function CarrinhosCreator({ dados }: { dados: CreatorPortalDados }) { const [carrinhos, setCarrinhos] = useState<CarrinhoPartilhavelCreator[]>([]); const [titulo, setTitulo] = useState(""); const [itens, setItens] = useState(""); const [erro, setErro] = useState(""); const carregar = () => listarCarrinhosCreator().then((r) => setCarrinhos(r.carrinhos)); useEffect(() => { void carregar(); }, []); async function criar(evento: FormEvent) { evento.preventDefault(); setErro(""); try { const entradas = itens.split("\n").map((linha) => linha.split(",").map((valor) => valor.trim())).filter((item) => item[0] && item[1]).map(([slugLoja, codigoPeca, quantidade]) => ({ slugLoja, codigoPeca, quantidade: Math.max(1, Number(quantidade) || 1) })); await criarCarrinhoCreator({ parceiroId: dados.parceiros[0]?.id, titulo, itens: entradas }); setTitulo(""); setItens(""); await carregar(); } catch (falha) { setErro(falha instanceof Error ? falha.message : "Carrinho inválido."); } } return <section className="creator-panel"><div className="creator-panel-head"><div><span>Venda assistida</span><h2>Carrinhos partilháveis</h2></div><ShoppingCart size={20} /></div><form className="creator-content-form" onSubmit={criar}><label>Título<input value={titulo} onChange={(e) => setTitulo(e.target.value)} required /></label><label className="wide">Produtos no formato loja,código,quantidade<textarea value={itens} onChange={(e) => setItens(e.target.value)} placeholder={"minha-loja,PROD-01,1\noutra-loja,PROD-22,2"} required /></label>{erro && <p role="alert">{erro}</p>}<button type="submit">Criar carrinho</button></form>{carrinhos.map((item) => <div className="creator-row" key={item.id}><div><strong>{item.titulo}</strong><span>{item.visualizacoes} visualizações · {item.importacoes} importações</span></div><Link to={`/carrinhos/${item.codigo}`}><ExternalLink size={16} /></Link></div>)}{!carrinhos.length && <p className="creator-empty">Ainda não existem carrinhos partilháveis.</p>}</section>; }
