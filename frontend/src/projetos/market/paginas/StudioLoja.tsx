...
import { montarUrlPublicaLoja, obterDominioPublicoLojaConfigurado } from "../dominio/lojaSubdominio";
import {
...
import {
  calcularTaxa,
  criarFormAPartirConfiguracao,
  criarFormVazio,
  criarPayloadConfiguracao,
  formatarListaTexto,
  formatarZonasEntregaTexto,
  normalizarSlug,
  parseCatalogosPersonalizadosTexto,
  parseListaTexto,
  parseTabelaMedidasTexto,
  parseZonasEntregaTexto,
  resumirCatalogoLocal,
  selecionarProdutosCriticos,
  selecionarProdutosDestaque
} from "../studio-loja/modelo";
import type {
  AcessoLojaDigital,
...
const passosAssistente: Array<{
  ...
  { id: "operacao", titulo: "Operação", detalhe: "Team e automações", icone: <Settings2 size={16} /> },
...
const obterOrdemVitrinesPadrao = () => vitrinesEditaveis.map((vitrine) => vitrine.id);
...
export function PaginaLojaPublica() {
  ...
  const [form, setForm] = useState<FormLoja>(() => criarFormVazio(obterOrdemVitrinesPadrao()));
  ...
  async function carregar() {
    ...
    try {
      ...
      setForm(criarFormAPartirConfiguracao(dadosConfiguracao, obterOrdemVitrinesPadrao()));
      setMensagem("");
    } catch (erro) {
    ...
    <CrmPageMotion className="loja-admin-shell">
      ...
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-[1fr_.92fr]">
        ...
        <CrmSection
          ...
          description="Ações que ligam loja, atendimento, pedidos, agenda e Team sem dispersar a equipa."
        >
  ...
  return (
    ...
    >
      <div className="bz-market-studio-panel">
        ...
        <div className="st-table bz-market-studio-table" style={{ marginTop: "1rem" }}>
          {itens.length ? (
...
function StudioSeguidoresPanel() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      setCarregando(true);
      try {
        const resposta = await listarSeguidoresTeam({ limite: 50 });
        if (!ativo) return;
...
function StudioMetricasPanel() {
  const [metricas, setMetricas] = useState<MetricasLojaTeam | null>(null);
  const [carregando, setCarregando] = useState(true);
  const [erro, setErro] = useState("");
  ...
  useEffect(() => {
    ...
    async function carregar() {
      setCarregando(true);
      try {
        const resposta = await obterMetricasLojaTeam();
        if (!ativo) return;
  ...
  return (
    <div className="edcard">
      <div className="edcard-head">
        <span className="studio-dot info" />
        <div>
          <h3>Presença pública</h3>
          <p>Escolha o ramo certo antes de publicar produtos: físico no Market, digital no Learning ou ambos.</p>
        </div>
      </div>
      <div className="qact">
        <button
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
      <CrmCommandPanel
        ...
        description={
          lojaConfigurada
            ? "Este negócio já tem uma loja digital. Por agora, o Team gere uma loja principal por negócio; use a configuração para ajustar a loja ativa."
...
function PassoOperacaoLoja({
  ...
  return (
    <section className="grid gap-5">
      ...
      <BlocoFormulario icon={<MousePointerClick size={18} />} title="Clientes e segmentação" detail="Ferramentas para importar, filtrar, editar e falar com clientes certos sem sair do Team.">
        <div className="grid gap-3 md:grid-cols-3">
      ...
      <BlocoFormulario icon={<Link2 size={18} />} title="Canais conectados" detail="Selecione as origens que a loja deve medir e encaminhar para as áreas certas do Team.">
        <div className="grid gap-3 md:grid-cols-2">
        ...
        <div className="st-phone-screen" style={{ color: "#0a0a0a" }}>
          <div className="relative h-28 bg-neutral-100">
            ...
            <div className="absolute bottom-3 left-3 flex items-center gap-2">
              <div className="grid size-12 place-items-center overflow-hidden rounded-xl border border-white/70 bg-white">
