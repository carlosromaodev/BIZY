...
import { persistirValorMedia } from "../../../../infra/media/MediaStorage.js";
...
const SubmeterFormularioLeadPublicoSchema = z.object({
  nome: z.string().trim().min(2).max(120),
  telefone: z.string().trim().min(8).max(30),
  email: z.string().trim().email().nullable().optional().transform((valor) => valor ?? null),
  produtoInteresse: z.string().trim().max(160).nullable().optional().transform((valor) => valor ?? null),
  mensagem: z.string().trim().max(2000).nullable().optional().transform((valor) => valor ?? null),
  consentimentoDados: z.literal(true),
  consentimentoMarketing: z.boolean().optional().default(false),
  trackingId: z.string().trim().max(120).nullable().optional().transform((valor) => valor ?? null),
  origem: z.string().trim().max(120).nullable().optional().transform((valor) => valor ?? null),
  canal: z.string().trim().max(120).nullable().optional().transform((valor) => valor ?? null)
});
...
export const moduloLojaPublica: ModuloHttp = {
  ...
  registrar(app, contexto) {
    ...
    app.post("/publico/lojas/:slug/formularios/lead", async (request, reply) => {
      const { slug } = request.params as { slug: string };
      aplicarNoStore(reply);
      const dados = SubmeterFormularioLeadPublicoSchema.parse(request.body ?? {});
      const resultado = await contexto.lojaPublica.registrarFormularioLead(slug, dados);
