...
export class RepassesFinanceirosUseCase {
  ...
  async resumoFinanceiroAdmin(): Promise<{
    ...
    const repasses = await this.deps.repassesFinanceiros.listarTodos();
    const fornecedores = new Map<string, {
      negocioId: string;
      pendente: number;
      conciliado: number;
      aprovado: number;
    ...
    for (const repasse of repasses) {
      const fornecedor = fornecedores.get(repasse.negocioId) ?? {
        negocioId: repasse.negocioId,
        pendente: 0,
        conciliado: 0,
        aprovado: 0,
        pago: 0,
      ...
      const chave = this.chaveEstadoRepasse(repasse.estado);
      fornecedor[chave] += repasse.valorLiquidoEmKwanza;
      fornecedor.totalLiquido += repasse.valorLiquidoEmKwanza;
      fornecedor.quantidade += 1;
      fornecedores.set(repasse.negocioId, fornecedor);

      totais[chave] += repasse.valorLiquidoEmKwanza;
      totais.totalLiquido += repasse.valorLiquidoEmKwanza;
      totais.quantidade += 1;
    }

    return {
      fornecedores: [...fornecedores.values()].sort((a, b) => b.totalLiquido - a.totalLiquido),
      totais,
      atualizadoEm: new Date().toISOString()
    };
  }

  private chaveEstadoRepasse(estado: RepasseFinanceiro["estado"]): "pendente" | "conciliado" | "aprovado" | "pago" | "cancelado" {
    switch (estado) {
      case "CONCILIADO":
        return "conciliado";
      case "APROVADO":
