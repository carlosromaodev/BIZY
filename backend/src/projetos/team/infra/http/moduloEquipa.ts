...
const ModoSombraQuerySchema = z.object({
  contexto: z.enum(["PAINEL", "PEDIDOS", "CONVERSAS", "FINANCAS"]).default("PAINEL")
});

const FiltrosMembrosQuerySchema = z.object({
...
export const moduloEquipa: ModuloHttp = {
  ...
  registrar(app, contexto) {
    ...
    app.get("/equipa/membros/:id/modo-sombra", async (request, reply) => {
      const ctx = await exigirEquipa(contexto, request, reply, "equipa:gestao");
      if (!ctx) return;

      const { id } = ParamIdSchema.parse(request.params);
      const { contexto: contextoWidget } = ModoSombraQuerySchema.parse(request.query ?? {});
      const membro = await contexto.gestaoEquipa.obterMembroDetalhado(ctx.negocio.id, id);
      if (!membro) return reply.code(404).send({ erro: "MEMBRO_NAO_ENCONTRADO", mensagem: "Membro não encontrado neste negócio." });

      const [modulos, widgets, checklist] = await Promise.all([
        contexto.gestaoEquipa.obterModulosVisiveis(ctx.negocio.id, id),
        contexto.gestaoWorkflow.obterWidgetsContextuais(ctx.negocio.id, contextoWidget, id),
        contexto.gestaoEquipa.listarChecklistOnboarding(id)
      ]);
      await contexto.gestaoEquipa.registarVisualizacaoModoSombra(ctx.negocio.id, id, ctx.usuario.id);
    ...
    app.get("/equipa/onboarding-guiado", async (request, reply) => {
      const ctx = await exigirEquipa(contexto, request, reply, "equipa:ler");
      if (!ctx) return;

      const membro = await contexto.gestaoEquipa.obterMembroPorUsuario(ctx.negocio.id, ctx.usuario.id);
      if (!membro) return reply.code(404).send({ erro: "MEMBRO_NAO_ENCONTRADO", mensagem: "Membro não encontrado neste negócio." });
      return contexto.gestaoEquipa.obterOnboardingGuiado(ctx.negocio.id, membro.id);
    });
